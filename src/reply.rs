...
use cosmwasm_std::{Attribute, BankMsg, Binary, CosmosMsg, DepsMut, Env, Event, Reply, Response, SubMsg, Uint128};
use osmosis_std::types::osmosis::poolmanager::v1beta1::{MsgSwapExactAmountIn, SwapAmountInRoute};

use crate::{
    contract::execute::{build_burn_msg, token_denom}, msg::{reply_msg_id, ExecuteMsg}, state::{read_state, GenerateTicketReq, IcpChainKeyToken, GENERATE_TICKET_REQ, STATE}, types::{MintTokenPayload, RedeemAllBTC}, ContractError
};
...
pub fn reply_error(
    _deps: DepsMut, _env: Env, msg: Reply
)-> Result<Response, ContractError> {
    ...
    match msg.id {
        ...
        reply_msg_id::GENERATE_TICKET_REPLY_ID => {
            Ok(
                Response::new().add_event(
                    Event::new("GenerateTicketFailed")
                )
            )
        }

        reply_msg_id::SWAP_CKBTC_TO_ALLBTC_REPLY_ID => {
            let mint_ckbtc: MintTokenPayload = 
                serde_json::from_slice(msg.payload.as_slice())
                .map_err(|e| 
                    ContractError::CustomError(e.to_string()))?;
            Ok(Response::new()
                .add_event(Event::new("SwapCKBTCFailed").add_attributes(vec![
        ...
        reply_msg_id::SWAP_ALLBTC_TO_CKBTC_REPLY_ID => {
            let redeem_allbtc: RedeemAllBTC = 
                ...
                .map_err(|e| 
                    ContractError::CustomError(e.to_string()))?;
            Ok(Response::new()
                .add_event(Event::new("SwapAllBTCFailed").add_attributes(vec![
...
pub fn reply_mint_token(
    ...
    .add_submessage(
        ...
        .with_payload(serde_json::to_vec(&mint_token_payload).map_err(|e| ContractError::CustomError(e.to_string()))?)
    ))
}
...
fn reply_swap_ckbtc_to_allbtc(
    ...
    let bank_msg = BankMsg::Send {
        to_address: mint_token_payload.receiver.into_string(),
        amount: vec![cosmwasm_std::Coin {
            denom: token_denom(env.contract.address.to_string(), all_btc_denom),
            amount: Uint128::new(mint_token_payload.amount.parse().unwrap()),
        }],
    };
